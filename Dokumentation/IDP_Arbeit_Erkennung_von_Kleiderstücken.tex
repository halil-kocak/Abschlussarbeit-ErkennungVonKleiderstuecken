...
Die Anpassung der entsprechende Gewichte wird durch den Rückpropagierung er-möglicht. Damit die Gewichte angepasst werden können, ist es nötig das das Model Fehlerwert ein Minimum annimmt. Es ist bekannt das mit dem Gradienten der Funktion das Minimum erreichbar ist, welches die partielle Ableitung aller Vektoren darstellt. Dieser Vektor wird auch $\delta(f)$ genannt. Sie zeigt an jedem Punkt der Funktion, die Richtung der steilsten anstieg  (bzw. maximum), wobei  $-\delta(f)$ die Richtung der steilsten abstieg (bzw. minimum) der Funktion zeigt \cite{Vogl2021} (S.337). Allgemein wird der Gradientenverfahren folgendermaßen dargestellt:\\

\begin{center}
	$x^{i+1} = x^{i} - \eta * \delta(f(x^{i}))$
\end{center}

, wobei $x^{0}$ der Startpunkt ist und $\eta$ die Schrittweite, welches auch als die Lernrate beschrieben wird. Diese Verfahren lässt sich wie folgt bei den Gewichten integrieren.
...
\\
Somit kann das Neuronale Netz seine Gewichte anpassen, womit es sich optimieren kann. Die partielle Ableitung der Fehlerfunktion E, wird durch die verwendung der Kettenregeln bestimmt.

\begin{center}
	$\frac{\delta E(W)}{\delta w_{ij}} =\sum_{k=1}^{n}[\frac{\delta}{\delta w_{ij}}(\frac{1}{2}(Y_{i} - \hat{Y_{i}})^{2})]$
\end{center}

, wobei $Y_{i}$ der prognostizierter Wert ist und $\hat{Y_{i}}$ der erwartete Zielwert. Dabei muss man achten, dass die $Y_{i}$ Werte vom Anfang an bekannt ist. Somit berechnet das Modell den Fehler mit der Fehlerfunktion und wendet die partielle Ableitung auf diese. Dadurch werden Gewichte mit der oben genannte Formel angepasst.   \cite{Vogl2021} (S.337-338). 
...
Der entweder den maximalen Wert x zurück gibt oder eine null wenn es einen anderes Maximum schon existiert. Für die Ausgabeschicht wird die Aktivierungsfunktion softmax eingestellt. Damit wird das erste Modell finalisiert und kann trainiert werden.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=15cm,height=9.5cm]		{ExperimentModelBewertungsgraf_1.png}
...
In diesem Experiment wird das Modell aus dem Experiment 1 übernommen und weiter verbessert. Das Ziel ist zunächst die reduzierung der Differenz von $val\_accuracy$ und accuracy, damit das Modell genauere Bewertung liefern kann. Dafür wird die Dropout Schicht benutzt, welches die Daten eliminiert, die zu einer Überanpassung des Modells führen. In diesem Layer werden Datensatze die mit einer Häufigkeit von rate auftreten, auf 0 gesetzt. Dabei ist rate der Parameter von dem Dropout Layer \cite{tensorflow2022}.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=15cm, height=9cm]		{ExperimentModelTrainingsphase_2.png}
	\caption{ Bewertung Trainingsphase \cite{HK22}(Experiment 2)}
	\label{fig:fig16}
\end{figure}

Es wird deutlich das die Genauigkeit der Trainingsdaten in diesem Model, mit der, die Genauigkeit der Validierungsdaten übereinstimmt. Somit wird die Überanpassung der Trainingsdaten verhindert, womit das Modell genauere Berechnungen führen kann. Die Differenz der Genauigkeit $accuracy$ und $val\_accuracy$ wurde von $2\%$ auf  $0.38\%$ reduziert. Somit sind die Genauigkeiten angepasst. 

\subsection{Experiment 3}

TODO: Änderung bergründen! Was ist anders?
