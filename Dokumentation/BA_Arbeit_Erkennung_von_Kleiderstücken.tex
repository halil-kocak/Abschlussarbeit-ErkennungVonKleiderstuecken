...
\includepdf[scale=1]{Abschlusserklaerung}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=8cm,height=6cm]{Abbildung_NeuronaleNetze_2.jpg}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=10cm,height=5cm]		{Abbildung_DeepLearning_1.jpg}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=15cm,height=5cm]		{Abbildung_CNN_1.png}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=14cm,height=16cm]		{ExperimentModelTrainingsphase_3.png}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=10cm,height=7cm]		{ExperimenteEval1.png}
...
\begin{table}[h!]
	...
	\end{center}
\end{table}
Anhand der Tabelle  \ref{tab:differenz2} ist ersichtlich, dass sich das Modell, für fast alle Klassen verbessert hat. Zunnächst werden die Trefferquoten beschrieben, da diese sich zur Bewertung der einzelnen Klassen besser eignen. Eine sehr große Änderung ist bei der Klasse Coat bemerkbar, da sich hier die Leistung um $10\%$ erhöht hat. Es werden die Klassen betrachtet für die der Klassifizierer, im ersten Experiment, eine schlechte Leistung erbracht hat. Eines dieser Klassen war die Klasse Coat, die wie auch zuvor erwähnt verbessert wurde. Die Klassen Top und Shirt haben sich in der Präzision verbessert, wobei die Klasse Shirt auch eine Verbesserung bezüglich der Trefferquote aufweist. Die einzigen Verschlechterungen sind in den Klassen Coat und Sandal vorhanden, wobei Sandal sich in der Trefferquote verschlechtert und die Klasse Coat in der Präzision. Keine Änderungen sind für die Klasse Bag zu sehen. Im Abbild 4.12 ist auch ersichtlich, dass die Klasse Trouser eine  $100\%$ Trefferquote und Sandal eine $100\%$ Präzision erzielt. Das Modell erzielt nun viel bessere Leistungen und kann somit genutzt werden. Eine Optimierung des Modells ist im Rahmen der Modellierung weiterhin möglich. Das Modell aus dem dritten Experiment wird als Zielmodell festgelegt und in Kapitel 5 mit einem Modell aus dem Internet verglichen.
...
Bisher wurde das neuronale Netz anhand dem CRIPS-DM Modell erstellt, optimiert und evaluiert. Während dem Optimieren sind Probleme wie zum Beispiel die Überanpassung entstanden, was verhindert werden sollte um genauere Ergebnisse mit dem Modell zu erzielen. Dafür wurde das Tensorflow Framework für Python benutzt. Dabei enthält das Framework einige Schichten, die es erleichtern das Modell zu erstellen und zu optimieren. Ein Beispiel dazu sind die Conv2D-, Dense-, Flatten- und Dropout-Layer. Nun wird ein Modell aus dem Internet mit dem dritten Modell verglichen. Dafür betrachten wir das Modell von Chinmay Rane, welches auf der Plattform Kaggle, auf der Seite \url{https://www.kaggle.com/fuzzywizard/fashion-mnist-cnn-keras-accuracy-93} aufzufinden ist. Das Modell von Chinmay Rane wird wie folgt definiert. 
...
Dabei wird die Feature Extraktion drei mal angewendet. Die Schicht BatchNormalization beschreibt eine Technik für das Trainieren von Deep Learning Netzen, bei der die Eingabedaten für jede Mini-Batch standardisiert wird. Dadurch wird der Lernprozess stabilisiert und es werden weniger Epochen benötigt. Das Modell erzielt eine Trainingsleistung von $92.95\%$ Genauigkeit und einen Verlust von $18.89\%$, wobei unser Modell eine Trainingsleistung von $95.31\%$ und ein Verlust von $15.82\%$ vorweist. Im folgenden Abbild werden die Evaluationsmetriken von dem Modell von Chinmay Rane dargestellt.

\begin{figure}[h!]
	\centering
	\includegraphics[width=7cm,height=8cm]		{ExperimenteEvalChinmayRane.png}
	\caption{ Evaluierung \cite{Rane2019} (K.8)}
	\label{fig:fig50}
\end{figure}

Betrachten man die Klassen, die schlechte Metriken haben, so kann man von Chinmay Ranes Modell die Klassen Coat, Shirt, Top und Pullover auswählen. Das Modell erzielt für diese Klassen eine Trefferquote von $85\%$ bis $86\%$ und eine Präzision von $75\%$ bis $91\%$. Zunächst werden die Differenzen der Evaluationsmetriken anhand einer Tabelle beschrieben. Die folgende Tabelle zeigt mit positiven Werten, dass das Modell von Chinmay Rane eine bessere Leistung erzielt hat. Bei einem negativen Wert erzielt das Modell aus Experiment 3 eine bessere Leistung.\\
...
Durch die Tabelle \ref{tab:differenzChinmayRane} wird deutlicher für welche Klassen das Modell von Chinmay Rane besser abschneidet. Ein Beispiel ist die Klasse Shirt, für die das Modell von Rane eine bessere Trefferquote erzielt als das Experiment 3. Das Experiment 3 erbringt für die Klassen Top, Pullover und Coat eine bessere Trefferquote als das Modell von Rane.\\
\\
Es stellt sich die Frage wie dieses Neuronale Netz weiterhin verbessert werden und eventuell andere Problemstellungen lösen kann. Bisher waren die Eingabedaten Graustufenbilder, die eine Auflösung von $28*28$ Pixel haben. Wird davon ausgegangen, dass die Eingabedaten nicht nur Topansichten beinhalten, sondern auch andere Sichtweisen, wie zum Beispiel ein Foto mit dem Smartphone mit Farbtönen, so kann man das Neuronale Netz erweitern, um mit diesen Eingabedaten eine Klassifizierung zuführen. Somit kann dann das Neuronale Netz Bilder klassifizieren, die mit einem Smartphone gemacht wurden. Dadurch könnte der Service so erweitert werden, dass es nicht nur im Online-Retail genutzt werden kann, sondern auch in verschiedentlichen Bereichen. Ein Beispiel dazu könnte das Rücksenden der bestellten Waren vom Kunden sein. Es ist bekannt das Käufer bei Rücksendungen die Artikel, welche zurück geliefert werden sollen, auf einem Papier vermerken müssen. Hierzu bietet das Neuronale Netz eine Automatisierung an, indem der Käufer ein Bild von dieser Ware hochlädt und diese automatisch klassifiziert wird. Somit hat man nicht nur den Bereich Online-Retail abgedeckt, sondern könnte den Service überall anwenden, wo es sich um Retouren handelt, wie zum Beispiel der E-Commerce Bereich. 
\cleardoubleoddpage
